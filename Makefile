...
doc:
	R --vanilla --slave -e 'library(roxygen2); roxygenise()'
	-git add --all man/*.Rd

test:
	R CMD INSTALL --install-tests .
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "ExpDesign2021", "tests")); system.time(test_check("ExpDesign2021", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran ExpDesign2021_$(VERSION).tar.gz
	rm -rf ExpDesign2021.Rcheck/
