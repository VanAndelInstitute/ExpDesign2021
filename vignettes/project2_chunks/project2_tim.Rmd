...
is_human <- function(x, minHuman=0.03, maxHuman=0.2, maxMouse=0.03) { 
  if (x['mouse'] > maxMouse) return(FALSE)
  else if (x['human'] > minHuman & x['human'] < maxHuman) return(TRUE)
  else return(FALSE) 
}
  
# note redudancy
is_mouse <- function(x, mouse, minMouse=0.03, maxMouse=0.2, maxHuman=0.03) {
  if (human > maxHuman) return(FALSE)
  else if (mouse > minMouse & mouse < maxMouse) return(TRUE)
  else return(FALSE)
}
  
classified <- function(x) { 
  if (is_mouse(x) & is_human(x)) return(FALSE)
  else if (is_mouse(x) | is_human(x)) return(TRUE) 
  else return(FALSE) 
}
  
frac <- data.frame(fracByCell)
frac$classifiable <- apply(frac, 1, classified)
